...
function myTTSinit() {
 ...
 let voice;
 voice = voices.findIndex(e => e.lang === 'ja-JP' && e.localService === true);
 if (voice !== -1) mySpeaker.push(voices[voice]);
 voice = voices.findIndex(e => e.lang === 'ja-JP' && e.localService === false);
 if (voice !== -1) mySpeaker.push(voices[voice]);
 const option = e => `<option value="${e.lang}">
   ${e.localService ? 'ネイティブ' : 'クラウド'}日本語</option>
   `;
...
function myInit() {
  ...
  const optIndexHtml = `<li><a href="${backto}.html">前のディレクトリに戻る</a></li>`;
