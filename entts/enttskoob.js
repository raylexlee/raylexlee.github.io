...
function myTTSinit() {
 ...
 let i;
 for (i=0; i < voices.length; i++) if (voices[i].lang.startsWith('en')) mySpeaker.push(voices[i]);
  if (!localStorage.getItem('enttsVoice')) {
    const start_voice = 0;
    localStorage.setItem('enttsVoice',start_voice);
  }
 const lastVoice = parseInt(localStorage.getItem('enttsVoice'));
