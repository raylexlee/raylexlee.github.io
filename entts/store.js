let myGroup, myBook;
const book = {};
const lastBookInGroupStored = g => `lastENttsBookInGroup${g}`
const lastBookStored = `lastENttsBook`
let lastBook;
let lastGroup;
...
const deviceType = getDeviceType();
  ...
  const data = await fetchText(`entts.txt`);
...
const ReadAloud = () => { 
  ...
  window.location = `qmttsaloud.html?title=${myBook.value}`; 
}
