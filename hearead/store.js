let myGroup, myBook;
const book = {};
const lastBookInGroupStored = g => `lastQMttsBookInGroup${g}`
const lastBookStored = `lastQMttsBook`
let lastBook;
let lastGroup;
const optionGroup = g => `<option value="${g}" ${(g == lastGroup) ? 'selected' : ''}>${Language[g].join(' ')}</option>`;
...
const deviceType = getDeviceType();
  ...
  const data = await fetchText(`triples.txt`);
...
const ReadAloud = () => { 
  ...
  window.location = `qmttsaloud.html?title=${myBook.value}&lang=${myGroup.value}`; 
}
